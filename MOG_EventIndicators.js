...
/*:
 * @plugindesc (v1.0) 이벤트 위에 애니메이션 표시자를 띄웁니다.
 * @author Moghunter
 * 
 * @param Default Font Size
 * @text 디폴트 폰트 크기
 * @desc 디폴트 폰트 크기를 설정합니다.
 * @default 16
 * 
 ...
 * By Moghunter 
 * korean translation by Sinacs
 * https://mogplugins.wordpress.com
 * =============================================================================
 * 이벤트 위에 애니메이션 표시자를 띄웁니다.
 *
 * =============================================================================
 * IMAGES
 * =============================================================================
 * 표시자에 사용할 이미지는 img/eventindicators 폴더에 넣습니다.
 *
 * -----------------------------------------------------------------------------
 * ANIMATED (Frames)
 * -----------------------------------------------------------------------------
 * 애니메이션 프레임을 사용한다면 다음 규칙에 맞게 파일명을 설정합니다.
 *
 * FILENAME + _INDEX.png
 *
 * EG
 ...
 * 주석(설명)에 입력 - 기본
 * =============================================================================
 * 이벤트의 주석(설명)에 다음과 같이 입력하면 표시자가 활성화됩니다.
 *
 * ------------------------------------------------------------------------------
 *
 * indicator : FILENAME : X : Y
 *
 * FILENAME - 사용할 이미지 파일 이름
 * X - X좌표 표시자 위치 보정값
 * Y - Y좌표 표시자 위치 보정값
 *
 * =============================================================================
 ...
 * =============================================================================
 *
 * TEXT - 텍스트를 표시하려면 다음과 같이 입력합니다.
 *
 * text_indicator : TEXT
 *
 * ------------------------------------------------------------------------------
 * VARIABLE - 변수를 표시하려면 다음과 같이 입력합니다.
 *
 * variable_indicator : VARIABLE_ID
 *  
 * ------------------------------------------------------------------------------
 * RANGE VISIBILITY - 플레이어와의 거리에 따라 표시자를 보이게 하려면
 * 					  다음과 같이 입력합니다.
 *
 * range_indicator : RANGE
 *
 * ------------------------------------------------------------------------------
 * ANIMATED - 애니메이션을 사용하려면 다음과 같이 입력합니다.
 *
 * animated_indicator : FRAMES : ANIMATION_SPEED
 *
 * ------------------------------------------------------------------------------
 * FONT_SIZE - 폰트 크기를 조정하려면 다음과 같이 입력합니다.
 *
 * fontsize_indicator : X : Y
 *  
 * ------------------------------------------------------------------------------
 * BLEND MODE - 블렌드 모드를 설정하려면 다음과 같이 입력합니다.
 *
 * blendmode_indicator : BLEND_MODE
 *
 * ------------------------------------------------------------------------------
 * ROTATION - 회전 애니메이션을 활성화하려면 다음과 같이 입력합니다.
 *
 * rotation_indicator : ROTATION_SPEED
 * 
 * ------------------------------------------------------------------------------
 * SHAKE - 떨림 애니메이션을 활성화하려면 다음과 같이 입력합니다.
 *
 * shake_indicator : SHAKE_POWER
 * ------------------------------------------------------------------------------
 * PULSE - 펄스(커졌다 작아졌다) 애니메이션을 활성화하려면 다음과 같이 입력합니다.
 *
 * pulse_indicator : PULSE_POWER
 *
 * ------------------------------------------------------------------------------
 * FLOAT - 부유(떠다니는) 애니메이션을 활성화하려면 다음과 같이 입력합니다.
 *
 * float_indicator
 *
 * ------------------------------------------------------------------------------
 * SLIDE - 슬라이드 애니메이션을 활성화하려면 다음과 같이 입력합니다.
 *
 * slide_indicator
 *
 * ------------------------------------------------------------------------------
 * CLEAR - 표시자를 제거하려면 다음과 같이 입력합니다.
 *
 * clear_indicator
 *    
 * =============================================================================
 * 플러그인 명령 (PLUGIN COMMAND)
 * =============================================================================
 * 특정 이벤트에 대해 표시자를 나타내거나 숨기려면 아래 명령어를 사용하십시오.
 *
 * show_indicator : EVENT_ID
 * hide_indicator : EVENT_ID
 *
 * 모든 이벤트에 대해 표시자를 나타내거나 숨기려면 아래 명령어를 사용하십시오.
 *
 * show_all_indicators
 * hide_all_indicators
 *
 * =============================================================================
 * 플러그인 명령으로 표시자를 활성화하려면 다음을 입력합니다.
 *
 * ------------------------------------------------------------------------------
 *
